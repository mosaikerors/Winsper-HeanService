...
public class HeanController {
  ...
  public JSONObject findAllByUId(@PathVariable(value = "uId") Long uId) {
    ...
    for(Hean hean:heanList){
      JSONObject heanJson=new JSONObject();
      heanJson.put("hId",hean.getHId());
      heanJson.put("uId",hean.getUId());
      heanJson.put("createdTime",hean.getCreatedTime());
      heanJson.put("text",hean.getText());
      heanJson.put("longtitude",hean.getLongtitude());
      heanJson.put("latitude",hean.getLatitude());
      heanJson.put("height",hean.getHeight());
      heanJson.put("tag",hean.getTag());
      List<String> picUrls=hean.getPics();
      JSONArray array= JSONArray.parseArray(JSON.toJSONString(picUrls));
      heanJson.put("pics",array);
      heanArray.add(heanJson);
    }
